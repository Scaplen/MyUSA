...
def nearby_lightning(
    ...
    engine: WeatherEngine | None = None,
) -> dict[str, Any]:
    weather_engine = engine or WeatherEngine()
    try:
        payload, cache_meta = weather_engine._fetch_json(
            DEFAULT_LIGHTNING_URL,
            f"lightning-feed:{DEFAULT_LIGHTNING_URL}",
            60,
        ...
        for feature in features:
            ...
            distance = miles_between(lat, lon, slat, slon)
...
def bundle_for_zip(
    ...
    weather = engine.forecast_bundle(resolved["lat"], resolved["lon"])
    ...
    if include_lightning:
        weather["lightning"] = nearby_lightning(
            resolved["lat"],
            resolved["lon"],
            engine=engine,
        )
